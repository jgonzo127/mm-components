...
var mm_posts_ajax_filter = function( e, newPageVal, newTerm ) {
	...
	var $filterLinks = $this.parents( '.mm-posts-filter-wrapper' ).find( '.mm-posts-filter a' );
	var $pagination = $mmPosts.next( '.mm-posts-ajax-pagination-wrapper' ).find( '.pagination' );
	var filterStyle;
	var totalPages;
	var newTerm;
	var $termText;
...
var mm_posts_ajax_pagination = function( newTerm ) {
	...
	var $mmPostsLoop = $mmPosts.find( '.mm-posts-loop' );
	var $mmPostsFilter = $mmPosts.prev( '.mm-posts-filter-wrapper' ).find( 'mm-posts-filter' );
	var $paginationWrapper = $this.parents( '.mm-posts-ajax-pagination-wrapper');
	var $paginationLinks = $mmPosts.find( '.pagination' ).find( 'a' );
	var $responseObj;
	var newPageVal;
	var filterStyle;
	var newTerm;
	...
	if( filterStyle == 'links' ) {
		...
		if( $mmPostsFilter.find( 'li a').hasClass( 'mm-posts-filter-all') ) {
			newTerm = '';
		} else {
			newTerm = $mmPosts.prev( '.mm-posts-filter-wrapper' ).find( '.active a' ).text();
		}
	}

	if( filterStyle == 'dropdown' ) {

		if( mmPostsFilter.val() == -1 ) {
			newTerm = '';
		} else {
			newTerm = $mmPosts.prev( '.mm-posts-filter-wrapper select' ).val();
		}
	}
	...
	newPageVal = $this.find( '.pagination li.active a' ).text();
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.mm-posts-filter-wrapper' ).find( '.mm-posts-filter .cat-item a').on( 'click', mm_posts_ajax_filter );
	$( '.mm-posts-filter-wrapper' ).find( '.mm-posts-filter #term_dropdown' ).on( 'change', mm_posts_ajax_filter );
